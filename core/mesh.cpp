...
Mesh* CreateCylindre(int segments, float radius, float height)
{
	const uint32_t numVerts = 1 + segments;

	Mesh* m = new Mesh();
	m->m_positions.resize(numVerts*2);
	m->m_normals.resize(numVerts*2);
	m->m_indices.resize(segments*3*2+ segments * 6);

	float halfHeight = height / 2;

	m->m_positions[0] = Point3(0.0f, halfHeight,0.0f);
	m->m_positions[1] = Point3(0.0f, halfHeight, radius);

	m->m_positions[numVerts] = Point3(0.0f, -halfHeight, 0.0f);
	m->m_positions[numVerts+1] = Point3(0.0f, -halfHeight, radius);

	float normal = 1.0f;

	m->m_normals[0] = Vec3(0.0f, normal, 0.0f);
	m->m_normals[1] = Vec3(0.0f, normal, 0.0f);

	m->m_normals[numVerts] = Vec3(0.0f, -normal, 0.0f);
	m->m_normals[numVerts+1] = Vec3(0.0f, -normal, 0.0f);

	float factor = k2Pi/segments;

	for (int i = 1, k=0,l=segments*3; i <= segments; ++i)
	{
		uint32_t nextVert = (i + 1) % numVerts;
		...
		float x = Sin(float(i)*factor);
		float z = Cos(float(i)*factor);
		m->m_positions[nextVert] = Point3(radius*x,halfHeight,radius*z);
		m->m_normals[nextVert] = Vec3(0.0f, normal, 0.0f);
		//m->m_normals[nextVert] = Vec3(x, 0.0f, z);

		m->m_positions[numVerts+nextVert] = Point3(radius*x, -halfHeight, radius*z);
		m->m_normals[numVerts + nextVert] = Vec3(0.0f, -normal, 0.0f);
		//m->m_normals[numVerts + nextVert] = Vec3(x, 0, z);

		m->m_indices[k++] = 0;
		m->m_indices[k++] = i;
		m->m_indices[k++] = nextVert;

		m->m_indices[l++] = numVerts;
		m->m_indices[l++] = numVerts+nextVert;
		m->m_indices[l++] = numVerts+i;
	}
	for (int i = 1, k = 0, l = segments * 3; i <= segments; ++i)
	{
		uint32_t nextVert = (i + 1) % numVerts;
	...
	m->m_positions.push_back( Point3(0.0f, halfHeight, radius) );
	m->m_positions.push_back( Point3(0.0f, -halfHeight, radius) );

	m->m_normals.push_back( Vec3(0.0f, 0.0f, 1.0f));
	m->m_normals.push_back( Vec3(0.0f, 0.0f, 1.0f));

	for (int i = 0,k= segments * 6,j = sizeLast; i < segments; ++i)
	{
		uint32_t nextVert = (i + 1) % segments;

		float x = Sin(float(i)*factor);
		float z = Cos(float(i)*factor);
		m->m_positions.push_back( Point3(radius*x, halfHeight, radius*z) );
		m->m_positions.push_back( Point3(radius*x, -halfHeight, radius*z));

		m->m_normals.push_back(Vec3(x, 0.0f, z));
		m->m_normals.push_back(Vec3(x, 0.0f, z));

		m->m_indices[k++] = sizeLast + nextVert * 2 ;
		m->m_indices[k++] = sizeLast + i * 2;
		m->m_indices[k++] = sizeLast + i * 2 + 1;

		m->m_indices[k++] = sizeLast + i * 2 + 1;
		m->m_indices[k++] = sizeLast + nextVert * 2 + 1;
		m->m_indices[k++] = sizeLast + nextVert * 2;
	}
...
Mesh* CreateMeshforPoint(float radius)
{
	const Point3 vertices[24] =
	{
		/// front
