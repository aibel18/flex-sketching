...
/// Interaction
NvFlexExtAsset* NvFlexExtCreateSoftFromVoxel(const float* voxeles, int numVoxeles, float clusterSpacing, float clusterRadius, float clusterStiffness, float linkRadius, float linkStiffness, float globalStiffness, float clusterPlasticThreshold, float clusterPlasticCreep)
{
	// Switch to relative coordinates by computing the mean position of the vertices and subtracting the result from every vertex position
	// The increased precision will prevent ghost forces caused by inaccurate center of mass computations
	...
	*/
	...
	const int numParticles = numVoxeles;
	...
	// cluster particles into shape matching groups
	//int numClusters = CreateClusters(&samples[0], &priority[0], numParticles, clusterOffsets, clusterIndices, clusterPositions, clusterSpacing, clusterRadius);

	int numClusters = CreateClusters((Vec3*)voxeles, &priority[0], numParticles, clusterOffsets, clusterIndices, clusterPositions, clusterSpacing, clusterRadius);
	...
	*/
	...
	for (int i = 0; i < numParticles; ++i)
	{
		asset->particles[i * 4 + 0] = voxeles[i * 3 + 0];
		asset->particles[i * 4 + 1] = voxeles[i * 3 + 1];
		asset->particles[i * 4 + 2] = voxeles[i * 3 + 2];
		asset->particles[i * 4 + 3] = 1.0f;
	}
