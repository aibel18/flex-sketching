...
{
	s_context->bindSolidShaderOculus(lightPos, lightTarget, lightTransform, shadowMap, bias, fogColor, width, height);
}
...
/// Interaction
void DrawGpuMesh2(GpuMesh* m, const Matrix44& xform, const Vec4& color) { s_context->drawGpuMesh2(m, xform, color); }
...
void initDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO, int numParticles) {
	s_context->initDrawPoints(idVBO,indicesVBO,numParticles);
}
/// Interaction
void updatePoints(unsigned int idVBO, unsigned int indicesVBO, const float* particles, int* indices, int n) {
	s_context->updatePoints(idVBO,indicesVBO,particles,indices,n);
}
/// Interaction
void drawPoints(unsigned int idVBO, unsigned int indicesVBO, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowMap) {
	s_context->drawPoints(idVBO,indicesVBO,  n,  offset, radius, screenWidth, screenAspect, fov, lightPos, lightTarget, lightTransform,shadowMap);
}
/// Interaction
void destroyDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO) {
	s_context->destroyDrawPoints(idVBO,indicesVBO);
}
...
void initDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer, int nWidth, int nHeight) {
	s_context->initDrawOculus(depthBufferID, renderTextureID, renderFrameBuffer, resolveTexture, resolveFrameBuffer, nWidth, nHeight);
}
/// Interaction
void drawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *matrix) {
	s_context->drawOculus(renderFrameBuffer, resolveTexture, resolveFrameBuffer,w,h,matrix);
}
/// Interaction
void finDrawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *matrix) {
	s_context->finDrawOculus(renderFrameBuffer, resolveTexture, resolveFrameBuffer, w, h, matrix);
}
/// Interaction
void destroyDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer) {
	s_context->destroyDrawOculus(depthBufferID, renderTextureID, renderFrameBuffer, resolveTexture, resolveFrameBuffer);
