...
public:
	...
	virtual void Initialize()
	{
		...
		for (int i = 0; i < int(mInstances.size()); i++)
			CreateSoftBody(mInstances[i], objects.size());
		/*
		if (mPlinth) 
			AddPlinth();
	...
	void CreateSoftBody(Instance instance, int group = 0)
	{
		...
		NvFlexExtAsset* asset = NvFlexExtCreateSoftFromMesh(
			(float*)&renderingInstance.mMesh->m_positions[0], /// mesh  vertices
			renderingInstance.mMesh->m_positions.size(), /// mesh	size vertices
			(int*)&renderingInstance.mMesh->m_indices[0], /// mesh indices
			renderingInstance.mMesh->m_indices.size(), /// mesh size indices

			mRadius,	/// Sampling Built Particles : distance
			instance.mVolumeSampling, /// Sampling Built Particles : volume
			instance.mSurfaceSampling,	/// Sampling Built Particles : superface

			instance.mClusterSpacing*mRadius,	/// cluster spacing
			instance.mClusterRadius*mRadius, /// cluster radius
			instance.mClusterStiffness,	/// cluster stiffness
			instance.mLinkRadius*mRadius,	/// link radius
			instance.mLinkStiffness,	/// link stiffness
			instance.mGlobalStiffness,	/// gloabal stiffness
	...
	virtual void Draw(int pass)
	{
		...
		{
			const RenderingInstance& instance = objects[s];
