...
class DemoContext
{
	...
	/// Interaction
	virtual void renderEllipsoidsOculus(FluidRenderer* renderer, FluidRenderBuffers* buffers, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap* shadowMap, Vec4 color, float blur, float ior, bool debug, int idFBO, int w, int h, Matrix44 view, Matrix44 proj) = 0;
	...
	/// Interaction
	virtual void bindSolidShaderOculus(Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap* shadowMap, float bias, Vec4 fogColor, int width, int height) = 0;
	...
	/// Interaction
	virtual void initDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO, int numParticles) = 0;
	/// Interaction
	virtual void updatePoints(unsigned int idVBO, unsigned int indicesVBO, const float* particles, int* indices, int n) = 0;
	///Interaction
	virtual void drawPoints(unsigned int idVBO, unsigned int indicesVBO, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowTex) = 0;
	/// Interaction
	virtual void destroyDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO) = 0;

	/// Interaction
	virtual void initDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer, int nWidth, int nHeight) = 0;
	/// Interaction
	virtual void drawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *matrix) = 0;
	virtual void finDrawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *matrix) = 0;
	/// Interaction
	virtual void destroyDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer) = 0;
	...
	/// Interaction
	virtual void drawGpuMesh2(GpuMesh* m, const Matrix44& xform, const Vec4& color) = 0;
