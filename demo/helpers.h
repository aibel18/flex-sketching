...
void CreateParticleShape(const Mesh* srcMesh, float spacing, Vec3 velocity, float invMass, bool rigid, float rigidStiffness, int phase, bool skin, float jitter=0.005f, Vec3 skinOffset=0.0f, float skinExpand=0.0f, Vec4 color=Vec4(0.0f), float springStiffness=0.0f)
{
	if (rigid && g_buffers->rigidIndices.empty())
		g_buffers->rigidOffsets.push_back(0);
...

	{
			...
			for (int y=0; y < maxDim; ++y)
			{
				for (int z=0; z < maxDim; ++z)
				{
					...
					if (voxels[index])
					{
						...
						Vec3 position = meshLower + spacing*Vec3(float(x) + 0.5f, float(y) + 0.5f, float(z) + 0.5f) + RandomUnitVector()*jitter;
		...
		ofstream file(GetFilePathByPlatform("../../data/controller.objx").c_str());

		if (!file)
			return;

		file << g_buffers->positions.size() /*<< " " << asset->numSprings*/ << endl;

		// add particle data to solver
		for (int i = 0; i < g_buffers->positions.size(); ++i)
		{
			file << g_buffers->positions[i][0] << " " << g_buffers->positions[i][1] << " " << g_buffers->positions[i][2] << endl;
		}
		// add link data to the solver 
		/*for (int i = 0; i < asset->numSprings; ++i)
		{
			file << asset->springIndices[i * 2 + 0] << " " << asset->springIndices[i * 2 + 1] << endl;
		}*/
		file.close();


		mesh.Transform(ScaleMatrix(1.0f + skinExpand)*TranslationMatrix(Point3(-0.5f*(meshUpper+meshLower))));
		mesh.Transform(TranslationMatrix(Point3( 0.5f*(meshUpper+meshLower))));	
