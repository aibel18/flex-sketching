...
/// Interaction
void initDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO, int numParticles) {
	// vbos
	glVerify(glGenBuffers(1, idVBO) );
	glVerify(glBindBuffer(GL_ARRAY_BUFFER, *idVBO));
	glVerify(glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * numParticles, 0, GL_DYNAMIC_DRAW));

	glVerify(glGenBuffers(1, indicesVBO));
	glVerify(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, *indicesVBO));
	glVerify(glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(int)*numParticles, 0, GL_DYNAMIC_DRAW));
}
void updatePoints(unsigned int idVBO, unsigned int indicesVBO, const float* particles,int* indices, int n) {

	// regular particles
	glVerify(glBindBuffer(GL_ARRAY_BUFFER, idVBO));
	glVerify(glBufferSubData(GL_ARRAY_BUFFER, 0, n * sizeof(Vec3), particles));

	glVerify(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indicesVBO));
	glVerify(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, n * sizeof(int), indices));
...
/// Interaction
void drawPoints(unsigned int idVBO, unsigned int indicesVBO, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowMap)
{
	static int sprogram = -1;
	if (sprogram == -1)
	{
	...
	if ( sprogram)
	{
		...
		glVerify(glUniform1f(glGetUniformLocation(sprogram, "spotMax"), g_spotMax));
		glVerify(glUniform1i(glGetUniformLocation(sprogram, "mode"), mode));
		glVerify(glUniform4fv(glGetUniformLocation(sprogram, "colors"), 8, (float*)&myColors[4].r));
...
/// Interaction
void destroyDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO) {
	glDeleteBuffers(1, idVBO);
	glDeleteBuffers(1, indicesVBO);
	//glVerify(glDeleteFramebuffers(1, idVBO));
}


/// Interaction
void initDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer, int nWidth, int nHeight) {
...
/// Interaction
void drawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *m ) {
...
/// Interaction
void finDrawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *m) {
...
void destroyDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer) {
...
/// Interaction
void BindSolidShaderOculus(Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowMap, float bias, Vec4 fogColor,int width, int height)
{
...
void RenderEllipsoidsOculus(FluidRenderer* render, FluidRenderBuffers* buffersIn, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowMap, Vec4 color, float blur, float ior, bool debug, int idFbo,int w, int h)
{
...
#if !ENABLE_SIMPLE_FLUID
	// resolve msaa back buffer to texture
	glVerify(glBindFramebuffer(GL_READ_FRAMEBUFFER_EXT, idFbo));
	glVerify(glBindFramebuffer(GL_DRAW_FRAMEBUFFER_EXT, render->mSceneFbo));
	glVerify(glBlitFramebuffer(0, 0, GLsizei(screenWidth), GLsizei(screenWidth / screenAspect), 0, 0, GLsizei(screenWidth), GLsizei(screenWidth / screenAspect), GL_COLOR_BUFFER_BIT, GL_LINEAR));
	...
	glVerify(glBindFramebuffer(GL_FRAMEBUFFER, idFbo));
	glVerify(glEnable(GL_DEPTH_TEST));
	glVerify(glDepthMask(GL_TRUE));
	glVerify(glDisable(GL_BLEND));
	glVerify(glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA));
...
void DrawGpuMesh2(GpuMesh* m, const Matrix44& xform, const Vec4& color)
{
	if (m)
	{
		GLint program;
...
/// Interaction
void DemoContextOGL::initDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO, int numParticles) {
	OGL_Renderer::initDrawPoints(idVBO,indicesVBO, numParticles);
}
void DemoContextOGL::updatePoints(unsigned int idVBO, unsigned int indicesVBO, const float* particles, int* indices, int n) {
	OGL_Renderer::updatePoints(idVBO, indicesVBO, particles,indices,n);
}
/// Interaction
void DemoContextOGL::drawPoints(unsigned int idVBO, unsigned int indicesVBO, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap* shadowTex)
{
	OGL_Renderer::drawPoints(idVBO, indicesVBO, n, offset, radius, screenWidth, screenAspect, fov, lightPos, lightTarget, lightTransform, shadowTex);
}
/// Interaction
void DemoContextOGL::destroyDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO) {
	OGL_Renderer::destroyDrawPoints(idVBO,indicesVBO);
}

/// Interaction
void DemoContextOGL::initDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer, int nWidth, int nHeight) {
	OGL_Renderer::initDrawOculus(depthBufferID,renderTextureID,renderFrameBuffer, resolveTexture, resolveFrameBuffer, nWidth,nHeight);
}
/// Interaction
void DemoContextOGL::drawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer,int w,int h,float *matrix) {
	OGL_Renderer::drawOculus(renderFrameBuffer, resolveTexture, resolveFrameBuffer,w,h,matrix);
}
/// Interaction
void DemoContextOGL::finDrawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *matrix) {
	OGL_Renderer::finDrawOculus(renderFrameBuffer, resolveTexture, resolveFrameBuffer, w, h, matrix);
}
/// Interaction
void DemoContextOGL::destroyDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer) {
	OGL_Renderer::destroyDrawOculus(depthBufferID, renderTextureID, renderFrameBuffer, resolveTexture, resolveFrameBuffer);
}
...
/// Interaction
void DemoContextOGL::bindSolidShaderOculus(Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap* shadowMap, float bias, Vec4 fogColor, int width, int height)
{
	OGL_Renderer::BindSolidShaderOculus(lightPos, lightTarget, lightTransform, shadowMap, bias, fogColor, width, height);
}
...
void DemoContextOGL::drawGpuMesh2(GpuMesh* m, const Matrix44& xform, const Vec4& color)
{
	OGL_Renderer::DrawGpuMesh2(m, xform, color);
}
