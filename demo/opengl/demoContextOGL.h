...
public:
	...
	/// Interaction
	virtual void renderEllipsoidsOculus(FluidRenderer* renderer, FluidRenderBuffers* buffers, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap* shadowMap, Vec4 color, float blur, float ior, bool debug, int idFBO, int w, int h, Matrix44 view, Matrix44 proj);
	...
	/// Interaction
	virtual void initDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO, int numParticles);
	///Interaction
	void updatePoints(unsigned int idVBO, unsigned int indicesVBO, const float* particles, int* indices, int n);
	///Interaction
	virtual void drawPoints(unsigned int idVBO, unsigned int indicesVBO, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowTex);
	/// Interaction
	virtual void destroyDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO);

	/// Interaction
	virtual void initDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer, int nWidth, int nHeight);
	/// Interaction
	virtual void drawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *matrix);
	virtual void finDrawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *matrix);
	/// Interaction
	virtual void destroyDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer);
	...
	/// Interaction
	virtual void bindSolidShaderOculus(Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap* shadowMap, float bias, Vec4 fogColor, int width, int height);
	...
	///Interaction
	virtual void drawGpuMesh2(GpuMesh* m, const Matrix44& xform, const Vec4& color);
