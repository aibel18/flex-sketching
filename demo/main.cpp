...
//Interaction
VRControl* vrcontrol;
void handleInputCallback(vr::VREvent_t event);
void handleInputJoystickCallback(int myEvent);
void createFrameCallback(FrameBuffer *fb, int w, int h);
void drawFrameCallback(FrameBuffer *fbL, int w, int h);
...
NvFlexBuffer* neighborsBuffer;
NvFlexBuffer* countsBuffer;
NvFlexBuffer* apiToInternalBuffer;
NvFlexBuffer* internalToApiBuffer;
...
// Interaction
/**
| Function that update the particles of simulation
*/
void updateHandModel() {
	...
	for(int iHand=0;iHand<2;iHand++){
		HandModel &rh = vrcontrol->hand[iHand];
		if (rh.sizeParticlesHand > 0)
			for (int i = 0, k = rh.indexParticlesHand; i < 5; i++) {
				for (int j = 0; j < 5; j++, k++) {
					g_buffers->positions[k][0] = rh.fingers[i].joints[j].x;
					g_buffers->positions[k][1] = rh.fingers[i].joints[j].y;
					g_buffers->positions[k][2] = rh.fingers[i].joints[j].z;
				}
			}
	}
...
// Interaction
int modelhand = 0;
bool exporthand = false;
void exportMyModelHand() {	

	char bufPath[255];
	sprintf(bufPath, "../../data/modelhand%d.txt", modelhand++);
	ofstream file(GetFilePathByPlatform(bufPath).c_str());

	if (!file)
		return;

	/// Right Hand
	HandModel &rh = vrcontrol->hand[0];
	for (int i = 0, k = rh.indexParticlesHand; i < 5; i++) {
		for (int j = 0; j < 5; j++, k++) {
			file <<"{" << rh.fingers[i].joints[j].x << "," << rh.fingers[i].joints[j].y << "," << rh.fingers[i].joints[j].z << "},";
		}
		file << endl;
	}
	file.close();
...
void Init(int scene, bool centerCamera = true)
{
	...
	/// Interaction
	int neighboursSize = maxParticles * g_maxNeighborsPerParticle;

	neighborsBuffer = NvFlexAllocBuffer(g_flexLib, neighboursSize, sizeof(int), eNvFlexBufferHost);
	countsBuffer = NvFlexAllocBuffer(g_flexLib, maxParticles, sizeof(int), eNvFlexBufferHost);;
	apiToInternalBuffer = NvFlexAllocBuffer(g_flexLib, neighboursSize, sizeof(int), eNvFlexBufferHost);;
	internalToApiBuffer = NvFlexAllocBuffer(g_flexLib, neighboursSize, sizeof(int), eNvFlexBufferHost);
	/// Interaction
	...
	if (g_warmup)
	{
		...
		const int kWarmupIterations = 10;//100;
...
void Shutdown()
{
	...
	/// Interaction
	NvFlexFreeBuffer(neighborsBuffer);
	NvFlexFreeBuffer(countsBuffer);
	NvFlexFreeBuffer(apiToInternalBuffer);
	NvFlexFreeBuffer(internalToApiBuffer);
				...
				for (int i = 0; i < numParticles; ++i)
				{
					...
					if ((sqr(x) + sqr(y)) <= (emitterWidth / 2)*(emitterWidth / 2))
					{
						...
						if (activeCount < g_buffers->positions.size())
						{
							...
							//g_buffers->velocities[activeCount] = Vec3(0, 0, 0);
...
void RenderScene()
{
	...
	g_lightDir = Normalize(Vec3(1.0f, 15.0f, 1.0f));
	...
	///Interaction
	g_scenes[g_scene]->Draw(radius,aspect, fov, lightTransform);
...
void MyRenderScene(Matrix44 proj, Matrix44 view, int width, int height, int idFbo) {
	...
	BindSolidShaderOculus(g_lightPos, g_lightTarget, lightTransform, g_shadowMap, 0.0f, Vec4(g_clearColor, g_fogDistance), width, height);
...
void UpdateFrame()
{
	...
	/// Interaction
	NvFlexGetNeighbors(g_solver, neighborsBuffer, countsBuffer, apiToInternalBuffer, internalToApiBuffer);
...
/// map event 
void handleInputCallback(vr::VREvent_t event) {
	g_scenes[g_scene]->EventController(event);
}
...
void createFrameCallback(FrameBuffer *fb,int w, int h) {
	::initDrawOculus(&fb->m_nDepthBufferId,&fb->m_nRenderTextureId,&fb->m_nRenderFramebufferId, &fb->m_nResolveTextureId,&fb->m_nResolveFramebufferId,w,h);
	fb->active = true;

}
void drawFrameCallback(FrameBuffer *fb, int w, int h) {
	//printf("Render\n");

	//float aspect = float(w) / h;

	Matrix44 proj(fb->matrixProj);
	Matrix44 view(fb->matrix);
	//Matrix44 view(vrcontrol->mHMD);
	::drawOculus(fb->m_nRenderFramebufferId, fb->m_nResolveTextureId, fb->m_nResolveFramebufferId, w, h, fb->matrix);	

	MyRenderScene(proj,view, w, h, fb->m_nRenderFramebufferId);
	
	::finDrawOculus(fb->m_nRenderFramebufferId, fb->m_nResolveTextureId, fb->m_nResolveFramebufferId, w, h, fb->matrix);
	
}
void destroyFrameCallback(FrameBuffer *fb) {
	::destroyDrawOculus(&fb->m_nDepthBufferId, &fb->m_nRenderTextureId, &fb->m_nRenderFramebufferId, &fb->m_nResolveTextureId, &fb->m_nResolveFramebufferId);
}
