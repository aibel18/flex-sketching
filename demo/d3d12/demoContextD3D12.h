...
public:
	...
	/// Interaction
	virtual void initDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO, int numParticles) override;
	///Interaction
	virtual void updatePoints(unsigned int idVBO, unsigned int indicesVBO, const float* particles, int* indices, int n) override;
	///Interaction
	virtual void drawPoints(unsigned int idVBO, unsigned int indicesVBO, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowTex) override;
	/// Interaction
	virtual void destroyDrawPoints(unsigned int *idVBO, unsigned int *indicesVBO) override;

	/// Interaction
	virtual void initDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer, int nWidth, int nHeight) override;
	/// Interaction
	virtual void drawOculus( unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *matrix) override;
	/// Interaction
	virtual void finDrawOculus(unsigned int  renderFrameBuffer, unsigned int resolveTexture, unsigned int resolveFrameBuffer, int w, int h, float *matrix) override;
	/// Interaction
	virtual void destroyDrawOculus(unsigned int *depthBufferID, unsigned int *renderTextureID, unsigned int * renderFrameBuffer, unsigned int *resolveTexture, unsigned int* resolveFrameBuffer) override;
	...
	/// Interaction
	virtual void bindSolidShaderOculus(Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap* shadowMap, float bias, Vec4 fogColor, int width, int height) override;
	...
	/// Interaction
	virtual void drawGpuMesh2(GpuMesh* m, const Matrix44& xform, const Vec4& color) override;
